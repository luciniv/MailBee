...
class Stats(commands.Cog):
    ...
    @commands.hybrid_command(
        name="hourly_data",
        description="View certain data types per hour from a selected day",
    )
    @checks.is_admin()
    @checks.is_guild()
    @app_commands.describe(type="Select a data type to display hourly")
    @app_commands.choices(
        type=[app_commands.Choice(name="Tickets opened & closed", value="open")]
    )
    ...
    @app_commands.describe(
        all_servers="Display data for all servers? (False by default)"
    )
    async def hourly_data(
        ...
        try:
            ...
            statsEmbed = discord.Embed(
                title=f"{time_value} Hourly Data: {day}-{month}-{year}",
                description=f"{type_name}{servers}\r{'⎯' * 30}",
                color=discord.Color.green(),
            )
            statsEmbed.set_author(name=guild.name, icon_url=guild.icon.url)

            query = queries.hourly_queries(type_value, guild_id, date_list, time_value)
    ...
    @app_commands.command(
        name="leaderboard", description="View certain data types as a leaderboard"
    )
    @checks.is_admin_app()
    @checks.is_guild_app()
    ...
    @app_commands.choices(
        type=[
            app_commands.Choice(
                name="Current tickets open (server ranking)", value="open"
            ),
            app_commands.Choice(
                name="Average ticket duration (server ranking)", value="duration"
            ),
            app_commands.Choice(
                name="Average first response time (server ranking)", value="response"
            ),
            app_commands.Choice(
    ...
    @app_commands.command(
        name="server_stats",
        description="Display this server's statistics,"
    ...
    async def server_stats(
        self, interaction, timeframe: discord.app_commands.Choice[str]
    ):
        try:
            # Allows command to take longer than 3 seconds
            await interaction.response.defer()
            ...
            query = queries.server_stats(guild_id, intervals)
            result = await self.bot.data_manager.execute_query(query)
            ...
            if result is not None:  # Go ahead to build embed
                ...
                statsEmbed.add_field(
                    name="📬 Tickets Open",
                    value=queries.format_data(data, index, None),
                    inline=True,
                )
                index += 2
                statsEmbed.add_field(
    ...
    @app_commands.command(
        name="mod_activity",
        description="Display a moderator's ticketing activity"
    ...
    async def mod_activity(
        ...
        try:
            ...
            query = queries.mod_activity(guild_id, close_id, intervals)
            result = await self.bot.data_manager.execute_query(query)
    ...
    @commands.hybrid_command(
        name="export_week", description="Output a CSV file of one week's data"
    )
    @checks.is_admin()
    @checks.is_guild()
    ...
    @commands.hybrid_command(
        ...
        description="Output a CSV file of every server's statistics,"
        " includes ticket counts and response averages",
    )
    @checks.is_admin()
    @checks.is_guild()
    ...
    async def export_server_stats(
        self, ctx, timeframe: discord.app_commands.Choice[str]
    ):
        try:
            # Allows command to take longer than 3 seconds
            await ctx.defer()
            ...
            statsEmbed = discord.Embed(
                title=f"Server Statistics Export",
                description=f"Download the attached CSV file to view data",
                color=discord.Color.green(),
            )
            statsEmbed.set_author(name=ctx.guild.name, icon_url=ctx.guild.icon.url)
            ...
            result_list = []
            query_list = queries.server_stats_CSV(guild_ids, intervals)
    ...
    @commands.hybrid_command(
        name="export_mod_activity",
        description="Output a CSV file of this server's moderators'"
    ...
    async def export_mod_activity(
        ...
        try:
            ...
            mod_ids_query = queries.get_mod_ids(guild_id, intervals)
            mod_ids_result = await self.bot.data_manager.execute_query(mod_ids_query)
            if len(mod_ids_result) != 0:
                for id in mod_ids_result:
                    mod_ids.append(id[0])

            statsEmbed = discord.Embeds(
                title=f"Moderator Activity Export",
                description=f"Download the attached CSV file to view data",
                color=discord.Color.green(),
            )
            statsEmbed.set_author(name=ctx.guild.name, icon_url=ctx.guild.icon.url)
            ...
            result_list = []
            query_list = queries.mod_activity_CSV(guild_id, mod_ids, intervals)
            ...
            if len(result_list) != 0:
                ...
                for mod_id, result in zip(mod_ids, result_list):
                    mod = await guild.fetch_member(mod_id)
                    data = [guild_id, mod_id, mod.name]
                    write_list.append((*data, *result))
